...
| 請求 | 相手のユーザーIDを指定して請求すると請求コード（例: `RQ-ABCD2345`）が発行され、相手がコードを入力すると支払いが成立する。請求状況（未払い／支払い済み／取り消し済み）も確認できる |
| 残高 | 保有しているすべての通貨の残高を一覧表示（**通貨は限定しない**。JPY/USD/EUR/… 任意のコードに対応） |
| プロフィール | 自分のユーザーIDを表示・コピー。相手に伝えると送金を受け取れる |
| 履歴 | 送金・受取の履歴を新しい順に表示 |
| ユーザー一覧 | ユーザーID検索、送金履歴からの保存、保存済みユーザーへの送金 |
| 割り勘リンク | 割り勘コード入りリンクを共有し、内容確認 → ログイン／登録 → 支払いへ進める |
| 割り勘 | 集金者がイベント名・人数・合計金額を登録すると請求コード（例: `SP-ABCD2345`）が発行される。参加者はコードを入力するとグループに参加し、割り勘後の金額が自動で請求される。支払いは**MegaPay残高**と**現金**（その場で渡した記録のみ・残高は動かさない）から選べ、集金者は誰がどちらで支払ったかを一覧で確認できる |
